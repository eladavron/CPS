...
public class DBController {
    ...
    {
        String manager = (managerID == 999 ? "CPS's Auto-Tasker" : employeeController.getEmployeeByID(managerID).getName());
        ...
        switch(reportType)
        {
            ...
            {
                shortDescription = "Daily Cancelled Orders";
            ...
            {
                shortDescription = "Daily Late Arrivals";
            ...
            {
                shortDescription = "Quarterly Orders";
        ...
        try {
            Statement stmt = db_conn.createStatement();
            shortDescription += " report. Generated by " + manager;
            stmt.executeUpdate(String.format("INSERT INTO AllReports (reportContent, shortDescription, dateGenerated)"
                            + " VALUES ('%s', '%s', '%s')",
                    sanitizeForSQL(reportToReturn),
                    sanitizeForSQL(shortDescription),
                    _simpleDateFormatForDb.format(new Date())));
        } catch (SQLException e) {
            if (IS_DEBUG_CONTROLLER) {
                System.err.println("An error occurred saving the report.");
            }
            throw e;
        }
    ...
    public ArrayList<FinalReport> queryAllReports() throws SQLException {
        ...
        while (rs.next())
        {
            Integer id = rs.getInt("idReport");
            String shortDescription = desanitizeFromSQL(rs.getString("shortDescription"));
            String content = desanitizeFromSQL(rs.getString("reportContent"));
            Date createdDate = new Date(rs.getTimestamp("dateGenerated").getTime());
            FinalReport finalReport = new FinalReport(id, shortDescription, content, createdDate);
            resultList.add(finalReport);
        }
        return resultList;
    }
