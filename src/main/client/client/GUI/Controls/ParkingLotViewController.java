...
public class ParkingLotViewController {
    ...
    private TabPane _parent;

    public ParkingLotViewController(ParkingLot parkingLot, TabPane parent) {
        this._parent = parent;
        this._parkingLot = parkingLot;
    }

    public void init(){
        for (int h = 0; h < _parkingLot.getHeight(); h++) //Get all spaces in a floor
        {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("ParkingLotFloor.fxml"));
                Tab newTab = loader.load();
                newTab.setText("Floor " + (h + 1));
                GridPane parkingGrid = (GridPane) loader.getNamespace().get("gridFloor");
                for (int w = 0; w < _parkingLot.getWidth(); w++) {
                    for (int d = 0; d < _parkingLot.getDepth(); d++) {
    ...
    private BorderPane parkingSpaceNode(ParkingSpace item) {
        ...
        if (item == null || item.getStatus() == null)
            pane.setCenter(new Label("Uninitialized"));
        else {
    ...
    {
        _parkingLot.getParkingSpaceMatrix()[space.getDepth()][space.getWidth()][space.getHeight()].setStatus(newStatus);
    }
}
