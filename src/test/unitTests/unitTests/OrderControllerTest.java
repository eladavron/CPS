...
class OrderControllerTest extends ApplicationTest {
    ...
    private Order simpleOrder = new Order(818, 777, addTime(date, TimeUtils.Units.DAYS, 2), parkingTemp);
    ...
    void finishSimpleOrderTest() throws SQLException {
        ...
        expected = orderController.makeNewSimpleOrder(simpleOrder.getCostumerID(),simpleOrder.getCarID(),simpleOrder.getEstimatedExitTime(),simpleOrder.getParkingLotNumber());

        customerController.addNewCustomer(customer);

        Billing.priceList priceType = customerController.getHourlyParkingCost(expected.getCostumerID(), expected);
        Order actual = orderController.finishOrder(expected.getOrderID(),priceType);//RAMI PLEASE FIX THE PRICE!!!
